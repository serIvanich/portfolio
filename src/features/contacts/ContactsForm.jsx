...
export const ContactsForm = ({ disabled, submitForm }) => {
  const formValidate = (values) => {
    ...
    if (values.phone && !/^\d{11}$/gi.test(values.phone)) {
  ...
  return (
    <form className={s.contactsFormContainer} onSubmit={formik.handleSubmit}>
      ...
      <input placeholder='' {...formik.getFieldProps('name')} />
      <div style={{ color: 'red', height: '10px' }}>
        {formik.touched.name && formik.errors.name && formik.errors.name}
      </div>
      <label htmlFor='email'>enter your email *</label>
      <input placeholder='' {...formik.getFieldProps('email')} />
      <div style={{ color: 'red', height: '10px' }}>
        {formik.touched.email && formik.errors.email && formik.errors.email}
      </div>

      <label htmlFor='phone'>enter your phone</label>
      <input placeholder='' {...formik.getFieldProps('phone')} />
      <div style={{ color: 'red', height: '10px' }}>
        {formik.touched.phone && formik.errors.phone && formik.errors.phone}
      </div>

      <label htmlFor='message'>write your text</label>
      <textarea placeholder='' {...formik.getFieldProps('message')} />
      <div style={{ color: 'red', height: '10px' }}>
